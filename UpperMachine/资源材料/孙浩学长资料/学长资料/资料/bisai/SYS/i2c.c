...
/*****************************i2c硬件接口******************************/
/*可以根据需要进行改变*/ 
#define    sda_pin    GPIO_Pin_2               //I2C  数据 
#define    scl_pin    GPIO_Pin_3               //I2C   时钟 
#define    iic_clk 	  RCC_APB2Periph_GPIOA	    //需要开启相应总线的时钟
#define    iic_group  GPIOA				        //接口所在分组

/********************************宏定义***********************************/
#define SCL(x)	x ? GPIO_SetBits(GPIOA , scl_pin)  : GPIO_ResetBits(GPIOA , scl_pin)
#define	SDA(x)  x ? GPIO_SetBits(GPIOA , sda_pin) : GPIO_ResetBits(GPIOA , sda_pin)
...
void SDA_IN()				
{
	...
	GPIO_Init(iic_group, &GPIO_InitStructure);           //初始化GPIOA端口
}
...
u8 i2c_Wait_Ack(void)
{
	...
	SCL(1);delay_us(1);	 
	while(GPIO_ReadInputDataBit(GPIOA ,sda_pin))
	{
		ucErrTime++;
		if(ucErrTime>250)
		{
  ...
  for(BitCnt=0;BitCnt<8;BitCnt++)
      {
        ...
        delay_us(1);
        retc=retc<<1;
        if(GPIO_ReadInputDataBit(GPIOA , sda_pin)== 1)retc=retc+1; // SDA == 1读数据位,接收的数据位放入retc中 
        delay_us(2);
...
u8 rcv_ack(void)
{
    ...
    SCL(1);                    //拉高时钟线
    delay_us(5);                 //延时
    bit=GPIO_ReadInputDataBit(GPIOA ,sda_pin);                   //读应答信号
    SCL(0);                    //拉低时钟线
    delay_us(5);                 //延时
    return bit;
