...
设置水平电机角度位置 单位为0.18度
******/
void PPM_H_PosSet(float Position)
{
	uint16_t CCR1_Val=0;
...
******/
void PPM_V_PosSet(float Position)
{
	uint16_t CCR4_Val=0;
...
void PPM_H_PWM_Set(uint16_t CCR_Val)
{
		TIM4->CCR2=CCR_Val;
}
void PPM_V_PWM_Set(uint16_t CCR_Val)
{
		TIM4->CCR4=CCR_Val;
}
